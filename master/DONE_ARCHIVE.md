...
## Archived 2026-05-07 (Run #22)

- [x] DONE [GROWTH] [S] Referral leaderboard at /waitlist/leaderboard — completion date: 2026-05-07 (Run #22). Public scoreboard, top 10 referrers ordered by `referrals_count DESC, id ASC` (earlier signup wins ties), email anonymized to first letter + domain (`alice@gmail.com` → `a***@gmail.com`). New `WaitlistLeaderboardService` (with package-private static `anonymize(email)` exposed for direct testing) and `WaitlistLeaderboardController` (GET `/waitlist/leaderboard` only — no POST surface). New `templates/waitlist-leaderboard.html` reuses the `waitlist-card` shell, renders a responsive `.leaderboard-table` with gold/silver/bronze rank pills (top 3 via `:nth-child`), and handles the empty state ("No referrers yet — be the first."). New repo derived query `findTop10ByReferralsCountGreaterThanOrderByReferralsCountDescIdAsc(int min)`. Linked from the waitlist success state's "See the referral leaderboard →". Advertised in `/sitemap.xml` (`changefreq=daily`, `priority=0.6`). 15 new tests across 4 files, including a triple-layer privacy contract that explicitly asserts raw email addresses cannot leak. Test count 161 → 176, BUILD SUCCESS. Public scoreboards 2-3× referral activity by adding social competition. (EPIC-1)

---

## Archived 2026-04-30 (Run #21)

- [x] DONE [GROWTH] [M] Pre-launch waitlist referral "skip the line" — completion date: 2026-04-30 (Run #21). V3 migration adds `referral_token VARCHAR(36)` (unique constraint, app-side `UUID.randomUUID()` generation in entity for portability across H2/Postgres) and `referrals_count INT NOT NULL DEFAULT 0` to `waitlist_entries`. New `WaitlistReferralService` exposes `creditReferrer(token, email)` (silently no-ops on blank/null/unknown tokens, rejects self-referral via case-insensitive email comparison) and `effectivePosition(entry)` (= `countByIdLessThan(id) + 1 - referralsCount * 100`, clamped at 1). `WaitlistController` accepts `?ref={token}` on GET (prefilled into the form's hidden field), calls the service on every successful POST, and exposes `referralUrl` (absolute, built from `app.base-url`), `position`, and `referralsCount` via flash attributes. `templates/waitlist.html` success and already-joined states render the referral block with copy-to-clipboard via the new `static/js/copy-button.js` (event-delegated, uses `navigator.clipboard.writeText` with `document.execCommand` fallback). 16 new tests across `WaitlistReferralServiceTest` (7 standalone Mockito), `WaitlistControllerTest` (5 new + 1 rewritten = 12 total), and `WaitlistEntryRepositoryTest` (5 new `@DataJpaTest`). Test count 145 → 161, BUILD SUCCESS. HIGH virality — the cheapest acquisition channel before billing exists. (EPIC-1)
- [x] DONE [UX] [S] Skip-the-line copy correctness — completion date: 2026-04-30 (Run #21). Fixed mathematically wrong "Refer 3 friends to jump 100 places ahead" (3 × 100 = 300, not 100) → "Each friend who joins from your link bumps you 100 places up the queue." Also fixed misleading inbound-referral banner "you'll both jump the line" (only the referrer skips on this signup, not the new user) → "🎁 A friend invited you. Sign up below — they'll jump 100 places, and you'll get your own link to share." (EPIC-1)
