...
_Last synced: 2026-05-07 (Run #22 — no shipped features since Run #21; spec content unchanged. Pre-launch waitlist referral "skip the line" mechanic still the most recent feature: /waitlist?ref={token} credits the referrer, success state shows position + share URL with copy-to-clipboard. /robots.txt + /sitemap.xml SEO endpoints; "Why MailIM" comparison section above the fold; error page dual-CTA; cookie banner on all 9 public templates; landing/pricing/demo/waitlist/privacy/terms/refund pages live; IMAP polling + Testcontainers + GreenMail integration tests in place; security headers filter active; gzip on; auth + Stripe + AI summary still planned)_
