...
- [ ] TODO [GROWTH] [S] "Your rank: #N" personalized overlay on /waitlist/leaderboard: when the visitor lands via `?ref=<theirToken>` (or carries a session/cookie marker once auth ships), look up their effective position and show a sticky banner above the table — "You're #N — 2 more referrals to crack the top 3." Personalized scoreboards convert 3-5× generic ones. Combine with a `(you)` highlight on their row in the table. MEDIUM impact, no prerequisites. (EPIC-1)
- [ ] TODO [GROWTH] [S] Real-time leaderboard auto-refresh: simplest version is `<meta http-equiv="refresh" content="60">` on /waitlist/leaderboard so the page reorders when shared in a Slack/Twitter context. SSE upgrade is optional. LOW-MEDIUM impact. (EPIC-1)
- [ ] TODO [GROWTH] [S] Leaderboard "share my rank" button: every leaderboard entry the visitor has just been credited for surfaces a one-click "Share my rank" button that pre-fills a tweet ("I'm #N on the @MailIM waitlist — bump me up: <my-ref-url>"). Turns each leaderboard view into an inbound traffic source. MEDIUM-HIGH virality, no prerequisites. (EPIC-1)
- [ ] TODO [GROWTH] [S] Extend the planned OG share-card generator to also render the user's current leaderboard rank (e.g. "I'm #4 on the MailIM waitlist"). Stacks with the existing share-card task. HIGH virality. (EPIC-1)
- [ ] TODO [GROWTH] [M] "Refer to unlock features" pre-launch tease: visible (but disabled) Personal/Team feature pills for users hitting referral milestones (5 = AI summary preview, 10 = SSO preview). Creates a feature-pull, not feature-push narrative. HIGH impact. Prerequisite: post-auth feature gating exists. (EPIC-5 prep)
- [ ] BLOCKED [GROWTH] [S] Weekly leaderboard email digest: send "You're #N this week. Top referrer crossed N invites — care to catch them?" to every entry with a non-zero rank. HIGH retention. [BLOCKED] until transactional email provider is chosen (TODO_MASTER.md). (EPIC-1)
...
- [ ] TODO [HEALTH] [S] Composite index on `waitlist_entries(referrals_count DESC, id ASC)` for the leaderboard query. Add as `V4__waitlist_leaderboard_index.sql` Flyway migration. At pre-launch volume this is a no-op (table scan + sort < 1ms), but past ~10K rows the LIMIT 10 query starts paying for the sort. Future-proofing only — not blocking. (EPIC-1)
