...
- [ ] [MARKETING] Once /press page ships (see INTERNAL_TODO.md), supply press-kit assets: 512×512 PNG logo (light + dark), 1280×800 product screenshot, founder headshot (300×300), one-paragraph elevator pitch, brand color hex codes. Without these the dev-side /press page will be a placeholder. The press kit is the single asset that turns "this looks cool" into "let me write about this" for journalists and roundup-post authors.
- [ ] [MARKETING] Once pre-launch waitlist referral is wired (see INTERNAL_TODO.md [GROWTH][M]), seed it: email the first 10 waitlist signups personally, share their referral URL, and ask if they'd send it to one productivity-minded friend. Pre-launch viral loops compound — the first 10 referrals become the next 100. Free, no tooling needed beyond your inbox.
- [ ] [MARKETING] Now that the "Why MailIM" inbox-vs-MailIM comparison is live on the landing page, take a screenshot of just the comparison block and post it on Twitter/X and LinkedIn with the caption "If your inbox feels like archaeology, here's why →" linking to /. Side-by-side comparison images outperform plain landing-page links 3-5× on engagement in productivity communities.
- [ ] [MARKETING] /sitemap.xml and /robots.txt are now live (Run #20). Once the production domain is up, register the property in Google Search Console (search.google.com/search-console) and Bing Webmaster Tools (bing.com/webmasters). Submit the sitemap URL (e.g. https://mailaim.app/sitemap.xml). Without this, the sitemap is invisible to search engines for weeks. Also set APP_BASE_URL to the production canonical URL (default is https://mailaim.app) — it's used to generate absolute `<loc>` entries in the sitemap.
- [ ] [MARKETING] Once /sitemap.xml is registered with Search Console, monitor "Coverage" and "Performance" reports weekly. The first 30 days of organic search data will reveal which keywords pull traffic to /pricing vs /demo vs /waitlist; that data should inform every subsequent SEO blog post topic.
- [ ] [MARKETING] Pre-launch waitlist referral feature is now LIVE (Run #21). Take a 15s screen recording of the /waitlist success state — show the "Skip the line" block, the auto-filled share URL, and the Copy-link button working — and post it on Twitter/X, IndieHackers, and LinkedIn with the caption "We just shipped a 'skip the line' referral mechanic for our pre-launch waitlist. Each friend you refer = 100 places ahead. Join → mailaim.app/waitlist". Pre-launch viral loops compound; the first 10 shares become the next 100.
- [ ] [MARKETING] Email each of the first 10 waitlist signups personally with their unique referral URL and a one-line ask: "If MailIM sounds useful to one productivity-minded friend, would you forward this link?" Hand-curated outreach beats any automated drip at this stage. Free, no tooling needed.
- [ ] [MARKETING] Once the production domain is up and APP_BASE_URL is set, share the canonical `https://{domain}/waitlist?ref={your-token}` URL in your own social profiles (Twitter/X bio, LinkedIn, IndieHackers). Founder-led referral seeding is the cheapest way to boot a viral loop.
- [ ] [MARKETING] Consider a "skip-the-line referrer prize": top 3 referrers at launch get a free year of the Personal plan. Costs $108 in foregone revenue per winner but creates a public competitive surface. Decide before announcing the referral mechanic publicly. NOW MORE TIME-SENSITIVE: the public leaderboard shipped Run #22 (/waitlist/leaderboard) — winners are now identifiable on a public canvas, which is the natural place to announce a prize.
- [ ] [MARKETING] Run #22 shipped a public referral leaderboard at /waitlist/leaderboard. Take a 30-60s screen recording showing the leaderboard reordering as a fresh signup arrives via `?ref=<token>` and post it on Twitter/X, IndieHackers, and LinkedIn with the caption "Public leaderboards 2-3× referral activity. Here's ours filling up live →". Motion is the hook — a static screenshot loses 80% of the impact of a live reorder. Use LICEcap, Kap, or a short OBS capture; no code required.
- [ ] [MARKETING] Founder-led leaderboard seed: refer 3-5 friends from your own referral URL on day one. The leaderboard's empty state ("No referrers yet — be the first.") is fine but a leaderboard with one or two entries already populated is far more inviting to climb than a blank one. Free, no tooling needed.
- [ ] [LEGAL] Once the waitlist scales beyond a few thousand entries, update /privacy to disclose that anonymized first-initial-plus-domain referral counts may appear on the public /waitlist/leaderboard page. For personal-domain users (e.g. `j***@johnsmith.com`) the domain is identity-adjacent. Acceptable for an opt-in waitlist at small scale; surface clearly in the privacy policy before scaling.
